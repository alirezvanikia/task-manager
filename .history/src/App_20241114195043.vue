...
export default {
  ...
  methods: {
    ...
    extractCompletedDates() {
      // فیلتر کردن تسک‌های تکمیل‌شده و دریافت تاریخ‌های آن‌ها
      this.completedDates = this.tasks
        .filter(task => task.isCompleted && task.completedTime)
        .map(task => task.completedTime);

      // نمایش تک تک تاریخ‌ها در کنسول
      this.completedDates.forEach(date => console.log(date));
    },
    addTask() {
  ...
  created() {
    ...
    this.fetchTasks(); // واکشی داده‌ها هنگام لود شدن صفحه
