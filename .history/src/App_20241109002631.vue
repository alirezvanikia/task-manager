...
export default {
  ...
  data() {
    return {
      ...
      newTaskLevel: '',
      tasks: [], // Tasks list 
      newTaskCategory: 'راست چین', // Default value to display "Rtl"
  ...
  computed: {
    activeTasks() {
      return this.tasks.filter(task => !task.isCompleted); // Filter active tasks
    },
    completedTasks() {
      return this.tasks.filter(task => task.isCompleted); // Filter completed tasks
    },
  },
  methods: {
    fetchTasks() { // Load tasks from JSON file
      fetch('/jsonData.json')
    ...
    addTask() {
      if (this.newTaskTitle) {
        // Add new task with personal id & empty input
        this.tasks.push({
    ...
    completeTask(completedTime, id) { // A function to mark the task as completed
    ...
    showTaskDetails(id) {
      ...
      const taskManagerDiv = this.$refs.taskManagerDiv; // Access to the task management element
      let overlayDiv = taskManagerDiv.querySelector('.overlay'); // Search for div with overlay class

      if (!overlayDiv) { // if there is no that div
        overlayDiv = document.createElement('div'); //Create new div
        overlayDiv.classList.add('overlay'); // Add overlay class to div
        overlayDiv.addEventListener('click', this.hideTaskDetails); // Click on and close modal
        taskManagerDiv.appendChild(overlayDiv); // Append overlayDiv in taskManagerDiv
      }
    },
    hideTaskDetails() { // A function to close the task details
      this.isModalVisible = false;
      const taskManagerDiv = this.$refs.taskManagerDiv;
      const overlayDiv = taskManagerDiv.querySelector('.overlay');
      if (overlayDiv) { // If there is an overlay
        taskManagerDiv.removeChild(overlayDiv);
      }
    },
  },
  mounted() { // The function that is executed when the component is loaded
    this.fetchTasks(); // Call fetchTasks to load tasks from JSON
  }
};
</script>
