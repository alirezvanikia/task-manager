...
export default {
  ...
  methods: {
    ...
    filterTasksByDate(startDate, endDate) {
        const start = new Date(startDate.replace(/\//g, '-')); // تبدیل به فرمت استاندارد
        const end = new Date(endDate.replace(/\//g, '-')); // تبدیل به فرمت استاندارد

        return this.tasks.filter(task => {
            if (task.completedTime) {
                const taskDate = new Date(task.completedTime.replace(/\//g, '-'));
                return taskDate >= start && taskDate <= end;
            }
            return false;
        });
    },
    handleEndTask(completedTime, taskId, elapsedTime) {
        const task = this.tasks.find(t => t.id === taskId);
        if (task) {
            task.completedTime = completedTime; // به‌روزرسانی مقدار والد
            task.elapsedTime = elapsedTime; // نگهداری زمان سپری شده
        }
    }
  },
  created() {
