              ...
              <div class="task-manager-card-container">
                <div class="task-manager-card-container">
                  <taskCard v-for="task in completedTasks" :key="task.id" :title="task.title" :type="task.type"
                    ...
                    :completedTime="task.completedTime" @detail-task="showTaskDetails(task.id)" @end-task="updateTaskCompletion" />
                </div>
...
export default {
  ...
  methods: { 
    ...
    completeTask(completedTime, id, completedElapsedTime) {
      ...
      if (task) { // بررسی وجود تسک
        ...
        console.log(completedElapsedTime);
      }
    },
    ...
    updateTaskCompletion({ id, completedTime, completedElapsedTime }) {
      // یافتن تسک و به‌روزرسانی زمان تکمیل و زمان سپری‌شده نهایی
      const task = this.tasks.find((task) => task.id === id);
      console.log(this.completedElapsedTime);
      
      if (task) {
        task.isCompleted = true;
        task.completedTime = completedTime;
        task.completedElapsedTime = completedElapsedTime;
      }
    },
  },
  created() {
