    ...
    <div :class="{ active: !isCompleted }" class="task-manager-card">
        ...
        <div class="task-manager-card__informations">
            ...
            <div v-else class="task-manager-card__timer">
                {{ localCompletedTime || 'تایم ثبت نشده' }} <!-- Show final or current time -->
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            localCompletedTime: this.completedTime, // نگهداری مقدار اولیه پراپ در متغیر محلی
    ...
    this.localCompletedTime = formattedDate;

    // ارسال داده به والد
    this.$emit('end-task', this.localCompletedTime, this.id, this.formattedTime);
},
    },
};
</script>