...
export default {
    ...
    const month = dateObj.getMonth() + 1; // ماه از 0 شروع می‌شود
    const year = dateObj.getFullYear();

    // ایجاد فرمت دلخواه
    const formattedDate = `${year}/${month}/${day}`;

    this.completedTime = formattedDate; // ذخیره در کامپوننت

    this.$emit('end-task', this.completedTime, this.id, this.formattedTime); // ارسال به والد
},
