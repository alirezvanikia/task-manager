    ...
    <div :class="{ active: !isCompleted }" class="task-manager-card">
        ...
        <div class="task-manager-card__informations">
            <div v-if="isCompleted" class="task-manager-card__timer">
                {{ completedElapsedHour }} <!-- Show final or current time -->
            </div>
            ...
            <div v-else class="task-manager-card__timer">
                {{ completedElapsedTime || 'Time not recorded' }} <!-- Show final or current time -->
            </div>
...
export default {
    props: {
        ...
        completedHour: { // Prop to display completion time
            type: String,
            default: ''
        },
        ...
        completedElapsedHour: { // The prop definition for the final elapsed time
            type: String,
            default: ""
        },
    ...
    methods: {
        ...
        endTask() {
            ...
            this.$emit('end-task', `${year}/${month}/${day}`, `${hours}:${String(minutes).padStart(2, '0')}`, this.id, this.formattedTime); // Time transfer
        },
