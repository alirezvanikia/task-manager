    ...
    <div :class="{ active: !isCompleted }" class="task-manager-card">
        ...
        <div class="task-manager-card__informations">
            ...
            <div v-else class="task-manager-card__timer">
                {{ isCompleted ? formattedCompletedTime : formattedTime }} <!-- Show final or current time -->
            </div>
...
// Finish the task and save the final time
        endTask() {
            ...
            this.$emit('end-task', this.compeletedTime, this.id); // Send completion event to parent
        },

        detailTask() {
            this.$emit('detail-task', this.id); // Send completion event to parent
        },
