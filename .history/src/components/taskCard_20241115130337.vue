<template>
    <div :class="{ active: !isCompleted }" class="task-manager-card">
        ...
        <div class="task-manager-card__informations">
            ...
            <div v-else class="task-manager-card__timer">
                {{ completedElapsedHour || 'تایم ثبت نشده' }} <!-- Show final or current time -->
            </div>
...
export default {
    ...
    methods: {
        ...
        endTask() {
            ...
            this.compeletedTime = `${year}/${month}/${day}`;
            this.compeletedHour = `${hours}:${minutes}`;
            this.$emit('end-task', this.compeletedTime, this.compeletedHour, this.id, this.formattedTime); // Send completion event to parent
        },
