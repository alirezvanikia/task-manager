            ...
            <div class="task-manager-history">
              <div class="task-manager-title">
                ...
                <div class="task-manager-filter">
                  <label for="dateFilter">فیلتر بر اساس تاریخ:</label>
                  <input type="date" id="dateFilter" v-model="selectedDate" @change="filterTasksByDate">
                </div>
              </div>
              <div class="task-manager-card-container">
                <div class="task-manager-card-container">
                  <taskCard v-for="task in filteredTasks" :key="task.id" :title="task.title"
              :type="task.type" :category="task.category" :completedTime="task.completedTime" />
    <div class="task-manager-box__empty" v-if="filteredTasks.length == 0">
      تسکی در این تاریخ یافت نشد!
    </div> 
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      completedDates: [], // آرایه‌ای برای ذخیره تاریخ‌های تکمیل‌شده
      selectedDate: null, // تاریخ انتخاب شده برای فیلتر
    };
  },
  computed: {
    activeTasks() {
    ...
    filteredTasks() {
      // اگر تاریخ انتخاب شده باشد، تسک‌ها را بر اساس تاریخ فیلتر می‌کند
      if (this.selectedDate) {
        return this.completedTasks.filter(task => {
          return task.completedTime && task.completedTime.startsWith(this.selectedDate);
        });
      }
      // اگر تاریخی انتخاب نشده باشد، تمام تسک‌های تکمیل‌شده را برمی‌گرداند
  ...
  methods: {
    fetchTasks() {
      fetch('/jsonData.json')
        ...
        .then(data => {
          ...
          this.extractCompletedDates(); // بعد از واکشی داده‌ها تاریخ‌ها را استخراج می‌کنیم
    ...
    completeTask(completedTime, id, elapsedTime) {
      ...
      const task = this.tasks.find(task => task.id === id); // Find a task with a given ID
      if (task) {
        task.isCompleted = true; // Set task status to completed
        task.completedTime = completedTime; // Record completion time
        task.elapsedTime = elapsedTime; // Record the elapsed time to complete the task
      }
    },
    ...
    extractCompletedDates() {
      this.completedDates = []; // ابتدا آرایه را خالی می‌کنیم تا اگر دوبار فراخوانی شد تکراری نشود

      // فیلتر کردن تسک‌های تکمیل‌شده و اضافه کردن تاریخ‌های آن‌ها به completedDates
      this.tasks.forEach(task => {
        if (task.isCompleted) {
          this.completedDates.push(task.completedTime); // اضافه کردن تاریخ به آرایه
          console.log(task.completedTime); // نمایش تاریخ در کنسول
        }
      });
    },
    filterTasksByDate() {
      // این متد در صورت نیاز می‌تواند برای تغییرات بیشتر به کار رود
      console.log("Tasks filtered by date:", this.selectedDate);
    },
  },
  created() {
