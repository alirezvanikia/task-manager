...
export default {
  ...
  if (this.selectedDate) {
    const formattedDate = this.selectedDate.replace(/-/g, '/'); // تبدیل تاریخ به فرمت صحیح
  ...
  watch: {
  tasks: {
    deep: true, // به دلیل اینکه tasks یک آرایه از آبجکت‌ها است، نیاز به deep watch داریم
    handler(newTasks) {
      this.extractCompletedDates(); // تاریخ‌های تکمیل‌شده را دوباره محاسبه کنید
    }
  }
},
  methods: {
  ...
  if (this.newTaskTitle) {
    const newTask = {
      ...
      isCompleted: false, // تسک جدید به صورت پیش‌فرض تکمیل نشده است
      completedTime: '' // زمان تکمیل برای تسک جدید خالی است
    };
    this.tasks.push(newTask);

    this.extractCompletedDates(); // به‌روزرسانی تاریخ‌های تکمیل‌شده
