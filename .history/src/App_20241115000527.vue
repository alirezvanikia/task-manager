...
export default {
  ...
  data() {
    return {
      ...
      selectedDate: null, // تاریخ انتخاب شده توسط کاربر
    };
  },
  computed: {
    activeTasks() {
    ...
    filteredCompletedTasks() {
      if (this.selectedDate) {
        // تبدیل selectedDate به فرمت YYYY/MM/DD
  ...
  methods: {
    ...
    addTask() {
    // اگر عنوان تسک جدید پر شده باشد، تسک جدید را اضافه می‌کنیم
    if (this.newTaskTitle) {
      const newTask = {
        id: Math.floor(Math.random() * 1000000), // تولید یک شناسه یکتا برای تسک
        title: this.newTaskTitle,
        type: this.newTaskType,
        category: this.newTaskCategory,
        level: this.newTaskLevel,
        description: this.newTaskDescription,
        isCompleted: false, // وضعیت عدم تکمیل برای تسک جدید
        completedTime: ''   // تاریخ تکمیل خالی تا وقتی که تسک تکمیل شود
      };
      
      this.tasks.push(newTask); // اضافه کردن تسک جدید به آرایه تسک‌ها
    ...
    completeTask(id) {
    const task = this.tasks.find(task => task.id === id); // پیدا کردن تسک با ID مشخص شده
    if (task) {
      task.isCompleted = true; // تنظیم وضعیت تسک به تکمیل شده

      // تنظیم تاریخ تکمیل به فرمت `YYYY/MM/DD`
      const today = new Date();
      const year = today.getFullYear();
      const month = String(today.getMonth() + 1).padStart(2, '0'); // تنظیم ماه به صورت دو رقمی
      const day = String(today.getDate()).padStart(2, '0'); // تنظیم روز به صورت دو رقمی

      task.completedTime = `${year}/${month}/${day}`; // تنظیم completedTime به فرمت `YYYY/MM/DD`
    }
  },
