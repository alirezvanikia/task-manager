<template>
  ...
  <taskCard
    ...
    :completed-elapsed-time="task.elapsedTime"
    @detail-task="showTaskDetails(task.id)"
...
export default {
  ...
  computed: {
    ...
    filteredCompletedTasks() {
      // اگر selectedDate پر باشد، تسک‌های تکمیل‌شده را فیلتر می‌کند
      if (this.selectedDate) {
        return this.completedTasks.filter(task =>
          task.completedTime && task.completedTime.startsWith(this.selectedDate)
        );
      }
      // اگر selectedDate خالی باشد، تمام تسک‌های تکمیل‌شده را برمی‌گرداند
  ...
  if (this.newTaskTitle) {
    ...
    this.tasks.push({
      ...
      isCompleted: false,
      createdTime: formattedDate // ذخیره تاریخ ایجاد تسک
    });
