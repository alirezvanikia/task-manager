...
export default {
  ...
  methods: { 
    ...
    //       this.tasks = data.map(task => ({ ...task, title: task.title,
    ...
    async fetchTasks() {
      try {
        const response = await axios.get('https://672ddbaffd8979715643fdfd.mockapi.io/api/v1/task-manager');
        this.tasks = response.data;
      } catch (error) {
        console.error('Error fetching tasks:', error);
      }

      axios.get("https://672ddbaffd8979715643fdfd.mockapi.io/api/v1/task-manager")
    .then(response => {
        console.log(response.data); // اینجا داده‌ها را چاپ کنید
        this.tasks = response.data;
    })
    .catch(error => console.error("Error fetching tasks:", error));
    ...
    async addTask() {
      if (this.newTaskTitle) {
        try {
          ...
          const response = await axios.post('https://672ddbaffd8979715643fdfd.mockapi.io/api/v1/task-manager', newTask);
          this.tasks.push(response.data); // اضافه کردن تسک به لیست محلی
    ...
    async completeTask(completedTime, id) {
    ...
    async deleteTask(id) {
      try {
        await axios.delete(`https://672ddbaffd8979715643fdfd.mockapi.io/api/v1/task-manager/${id}`);
...
<template>
  <div :class="{ active: !isCompleted }" class="task-manager-card">
      ...
      <div class="task-manager-card__informations">
          ...
          <div class="task-manager-card__timer">
              {{ formattedCompletedTime }} <!-- Show final or current time -->
          </div>
...
export default {
  ...
  methods: {
      ...
      endTask() {
          clearInterval(this.timer);
          this.completedElapsedTime = this.elapsedTime; // Save the final elapsed time
          ...
          this.$emit('end-task', this.compeletedTime, this.id, this.completedElapsedTime); // Send completion event to parent
      },
