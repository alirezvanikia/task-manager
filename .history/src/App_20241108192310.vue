    ...
    fetchTasks() {
      fetch('/jsonData.json')
        ...
        .then(data => {
          ...
          this.tasks = data.map(task => ({ ...task, title: task.title, category: task.category, type: task.type, level: task.level, description: task.description, title: task.title, isCompleted: task.completedTime ? true : false }));
        })
