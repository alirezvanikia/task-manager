...
export default {
  ...
  data() {
    return {
      ...
      completedDates: [] // آرایه برای ذخیره تاریخ‌های تکمیل
  ...
  methods: {
    ...
    collectCompletedDates() {
      // استخراج تاریخ‌های تکمیل شده از تسک‌ها و ذخیره در `completedDates`
      this.completedDates = this.tasks
        .filter(task => task.isCompleted && task.completedTime) // فقط تسک‌های تکمیل شده با تاریخ معتبر
        .map(task => task.completedTime); // استخراج تاریخ‌ها
        .forEach(task => console.log(task.completedTime));

        console.log('test : ',this.completedDates);
        
    }
  },
  created() {
    this.fetchTasks(); // فراخوانی داده‌ها هنگام لود شدن صفحه
    this.collectCompletedDates(); // جمع‌آوری تاریخ‌ها هنگام لود صفحه
  },
  mounted() {
    this.fetchTasks();
  }
